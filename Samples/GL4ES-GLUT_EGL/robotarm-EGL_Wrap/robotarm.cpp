...
void special(ESContext *esContext, int key, int, int) {
  switch (key) {
    case GLUT_KEY_4: (elbowAngle += 5) %= 360; break;
    case GLUT_KEY_6: (elbowAngle -= 5) %= 360; break;
    case GLUT_KEY_8: (shoulderAngle += 5) %= 360; break;
    case GLUT_KEY_2: (shoulderAngle -= 5) %= 360; break;
...
int main(int argc, char** argv) {
  ...
  glutCreateWindow(&esContext, "Robot Arm EGL GL4ES OS4");
