...
void myReshape(ESContext *esContext, int,int);
...
void myReshape(ESContext *esContext, int w, int h)
{
	width = w;
	height = h;
...
static void
key(ESContext *esContext, unsigned char k, int x, int y)
{
	switch (k) {
		case 27:  /* Escape */
...
int main(int argc, char** argv)
{
	...
	myinit();
//	  glutFullScreen();
	glutReshapeFunc (&esContext,myReshape);
