...
void oneFrame(ESContext *esContext)
{
...
void myinit(ESContext *esContext, int w, int h)
{
...
/* ARGSUSED1 */
void myKeyboardCallback(ESContext *esContext, unsigned char c, int x, int y)
{
	switch (c) {
		case 27:
		...
		case 'a':
			iShowFlags = c - '0';
            printf("Enabling disable iShowFlags\n");
            printf("myKeyboardCallback(a) == %d \n", c);
			break;

		case 'f':
			bShowFPS = !bShowFPS;
            printf("Enabling or disable ShowFPS\n");
            printf("myKeyboardCallback(f) == %d \n", c);
			break;

		case 't':
			if ( (bAnimate = !bAnimate) ) {
				printf("Enabling animation\n");
                printf("myKeyboardCallback(t) == %d \n", c);
			}
			else {
		...
		case 'o':
            ...
            printf("Enabling glPolygonMode(GL_FRONT_AND_BACK, GL_LINE)\n");
            printf("myKeyboardCallback(o) == %d \n", c);
			break;

		case 'p':
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            printf("Enabling glPolygonMode(GL_FRONT_AND_BACK, GL_FILL)\n");
            printf("myKeyboardCallback(p) == %d \n", c);
			break;
...
int main(int argc, char *argv[])
{
	...
	glutInitDisplayMode(fbmode | GLUT_RGBA | GLUT_DEPTH);
	glutInitWindowSize(width, height);
	glutCreateWindow(&esContext, argv[0]);
...
//		  glHint(MGL_W_ONE_HINT, GL_FASTEST);
		...
		myinit(&esContext, width, height);

		glutIdleFunc(&esContext,oneFrame);
		glutKeyboardFunc(&esContext,myKeyboardCallback);
