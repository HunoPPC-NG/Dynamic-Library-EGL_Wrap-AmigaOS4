...
extern t3DModel g_3DModel;                                     // This holds the 3D Model info that we load in
...
void CreateTexture(unsigned int textureArray[],char *strFileName,const GLuint textureID)
{
    ...
    // Build Mipmaps (builds different versions of the picture for distances - looks better)
     gluBuild2DMipmaps(GL_TEXTURE_2D, 3, pBitmap[0]->w, pBitmap[0]->h, GL_RGB, GL_UNSIGNED_BYTE, pixels);
       //      glTexImage2D(GL_TEXTURE_2D, 0, 3, pBitmap[0]->w, pBitmap[0]->h, 0, GL_RGB, GL_UNSIGNED_BYTE, pixels);
             free(pixels);
...
int main(void)
{
    ...
    CreateMyWindow("www.GameTutorials.com - ASE Loader", SCREEN_WIDTH, SCREEN_HEIGHT, VideoFlags);
