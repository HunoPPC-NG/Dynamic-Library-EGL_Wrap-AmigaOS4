...
void CreateTexture(unsigned int textureArray[],char *strFileName,int textureID)
{
    ...
    for(int i = 0; i < (width * height); ++i)
    {
        ...
        pixels[count++] = b;          // in our tImage classes we store r first
        pixels[count++] = g;          // then g
        pixels[count++] = r;          // and finally b (for bmps - three channels only)
        /*pixels[(i * BytesPerPixel) + 0] = b;          // in our tImage classes we store r first
        pixels[(i * BytesPerPixel) + 1] = g;          // then g
        pixels[(i * BytesPerPixel) + 2] = r;          // and finally b (for bmps - three channels only)*/
...
int main(void)
{
    ...
    CreateMyWindow("www.GameTutorials.com - Masking", SCREEN_WIDTH, SCREEN_HEIGHT, VideoFlags);
